...
impl MovePicker {
    ...
    pub const fn new_probcut(threshold: i32, special_move: Move) -> Self {
        Self {
            list: MoveList::new(),
            tt_move: Move::NULL,
            special_move,
    ...
    pub fn next<NODE: NodeType>(&mut self, td: &ThreadData, skip_quiets: bool, ply: isize) -> Option<Move> {
        ...
        if self.stage == Stage::GoodNoisy {
            ...
            self.stage = if !self.special_move.is_null() { Stage::SpecialMove } else { Stage::GenerateQuiet };
        }

        if self.stage == Stage::SpecialMove {

            self.stage = Stage::GenerateQuiet;

            if self.special_move.is_quiet() && td.board.see(self.special_move, self.threshold.unwrap()) && td.board.is_pseudo_legal(self.special_move) {
                return Some(self.special_move);
            }
        }
