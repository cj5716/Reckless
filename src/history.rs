...
impl NoisyHistoryEntry {
    ...
    pub fn bucket(&self, threats: Bitboard, from: Square, to: Square, captured: PieceType) -> i16 {
        let from_threatened = threats.contains(from) as usize;
        let to_threatened = threats.contains(to) as usize;
        self.buckets[captured][from_threatened][to_threatened]
    }
    ...
    pub fn update_bucket(&mut self, threats: Bitboard, from: Square, to: Square, captured: PieceType, bonus: i32) {
        let from_threatened = threats.contains(from) as usize;
        let to_threatened = threats.contains(to) as usize;
        let entry = &mut self.buckets[captured][from_threatened][to_threatened];
        apply_bonus::<{ Self::MAX_BUCKET }>(entry, bonus);
    }
}

pub struct NoisyHistory {
    // [piece][to][captured_piece_type][from_threatened][to_threatened]
    entries: Box<PieceToHistory<NoisyHistoryEntry>>,
}

impl NoisyHistory {
    pub fn get(&self, threats: Bitboard, piece: Piece, from: Square, to: Square, captured: PieceType) -> i32 {
        let entry = &self.entries[piece][to];
        (entry.factorizer + entry.bucket(threats, from, to, captured)) as i32
    }

    pub fn update(
        &mut self, threats: Bitboard, piece: Piece, from: Square, to: Square, captured: PieceType, bonus: i32,
    ) {
        let entry = &mut self.entries[piece][to];

        entry.update_factorizer(bonus);
        entry.update_bucket(threats, from, to, captured, bonus);
    }
}
