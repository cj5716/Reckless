...
fn search<NODE: NodeType>(
    ...
    if !NODE::ROOT && !excluded && potential_singularity && ply < 2 * td.root_depth as isize {
        ...
        if score < singular_beta {
            let double_margin =
                200 * NODE::PV as i32 - 16 * tt_move.is_quiet() as i32 - 27 * correction_value.abs() / 256;
            let triple_margin =
                288 * NODE::PV as i32 - 16 * tt_move.is_quiet() as i32 - 27 * correction_value.abs() / 256 + 32;
    ...
    while let Some(mv) = move_picker.next::<NODE>(td, skip_quiets, ply) {
        ...
        if depth >= 2 && move_count >= 2 {
            ...
            reduction -= 2699 * correction_value.abs() / 1024;
        ...
        // Full Depth Search (FDS)
            ...
            reduction -= 2131 * correction_value.abs() / 1024;
