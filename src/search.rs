    ...
    if best_move.is_present() {
        ...
        if best_move.is_noisy() {
            ...
            for (i, &mv) in quiet_moves.iter().enumerate() {
                ...
                let scale1 = 1024_i32 * 1024 / (denom * denom / 1024);

                let history = (td.quiet_history.get(td.board.all_threats(), stm, mv) + td.conthist(ply, 1, mv) + td.conthist(ply, 2, mv)).clamp(-8192, 8192);
                let scale2 = (history + 12402) / 16;

                td.quiet_history.update(td.board.all_threats(), stm, mv, -quiet_malus * (scale1 + scale2) / 2048);
                update_continuation_histories(td, ply, td.board.moved_piece(mv), mv.to(), -cont_malus * (scale1 + scale2) / 2048);
            }
        }
