...
pub fn start(td: &mut ThreadData, report: Report, thread_count: usize) {
    ...
    for depth in 1..MAX_PLY as i32 {
        ...
        for index in 0..td.multi_pv {
            ...
            let best_avg = ((td.shared.best_stats[td.pv_index].load(Ordering::Acquire) & 0xffff) as i32 - 32768 + average[td.pv_index]) / 2;
            ...
            loop {
                ...
                match score {
                    ...
                    _ => {
                        ...
                        td.shared.best_stats[td.pv_index].fetch_max(((depth as u32) << 16) | (average[td.pv_index] + 32768) as u32, Ordering::AcqRel);
