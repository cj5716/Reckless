...
pub fn activate_ft(pst: &PstAccumulator, threat: &ThreatAccumulator, stm: Color) -> Aligned<[u8; L1_SIZE]> {
    ...
    for flip in [0, 1] {
        ...
        for i in 0..L1_SIZE / 2 {
            ...
            let right = (pst_input[i + L1_SIZE / 2] + threat_input[i + L1_SIZE / 2]).clamp(0, (FT_QUANT << FT_ACT_SHIFT) as i16);

            output[i + flip * L1_SIZE / 2] = ((left as i32 * right as i32) >> (FT_SHIFT + FT_ACT_SHIFT)).clamp(0, FT_QUANT) as u8;
        }
    }
